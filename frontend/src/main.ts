...
function bootVoxelRoom(): void {
  ...
  function handleMessage(message: ServerMsg): void {
    if (message.kind === "Welcome") { you = message.you; host = message.snapshot.game.host; connected = true; players.clear(); mutedPlayers.clear(); message.snapshot.players.forEach((player) => players.set(player.id, player.name)); canvas?.replaceWorkspace(message.snapshot.voxels); messages.innerHTML = ""; message.snapshot.chat.forEach((line) => appendChat(players.get(line.player) ?? "Builder", line.text)); input.disabled = false; input.placeholder = "Say something..."; document.querySelector(".voxel-room")?.classList.remove("is-disconnected"); setGrid(message.snapshot.grid_size ?? 20); publishVoiceState(); renderPlayers(); return; }
    if (message.kind === "Presence") { message.joined.forEach((player) => players.set(player.id, player.name)); message.left.forEach((id) => { players.delete(id); mutedPlayers.delete(id); }); if (message.joined.length) publishVoiceState(); renderPlayers(); return; }
    ...
    if (message.kind === "Voice") { if (message.muted) mutedPlayers.add(message.player); else mutedPlayers.delete(message.player); renderPlayers(); return; }
