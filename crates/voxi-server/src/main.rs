...
async fn main() -> anyhow::Result<()> {
    let _ = dotenvy::from_path("crates/voxi-server/.env");
    let _ = dotenvy::dotenv();

    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,voxi=debug".into()),
        )
        .init();

    let data_dir: PathBuf = std::env::var_os("VOXI_WORDS_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from("crates/voxi-server/data"));
    ...
    tracing::info!(%addr, "voxi-server listening");
